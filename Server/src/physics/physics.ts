...
import { Vector3, norm, upCross, up, sin, cos } from "./utils"
import { muS, muC, g, m, Mz, Mxy, R, I, e, ee, μs, μw } from "./constants"
import { Mathaven } from "./mathaven"

export function surfaceVelocity(v: Vector3, w: Vector3): Vector3 {
  return surfaceVelocityFull(v, w).setZ(0)
}

const sv = new Vector3()
export function surfaceVelocityFull(v: Vector3, w: Vector3): Vector3 {
...
export function rotateApplyUnrotate(
  ...
  v: Vector3,
  w: Vector3,
  model: (v: Vector3, w: Vector3) => { v: Vector3; w: Vector3 }
) {
...
export function s0(v: Vector3, w: Vector3): Vector3 {
  return new Vector3(
...
export function c0(v: Vector3): number {
  return v.x * cos_a
}

export function Pzs(s: Vector3): number {
  const A = 7 / 2 / m
  return s.length() / A
}

export function Pze(c: number): number {
  const B = 1 / m
  const coeff = restitutionCushion(new Vector3(c / cos_a, 0, 0))
  return (muC * ((1 + coeff) * c)) / B
}

export function isGripCushion(v: Vector3, w: Vector3): boolean {
...
export function muCushion(v: Vector3): number {
  const theta = Math.atan2(Math.abs(v.y), v.x)
  return 0.471 - theta * 0.241
}

export function restitutionCushion(v: Vector3): number {
  const e = 0.39 + 0.257 * v.x - 0.044 * v.x * v.x
  return e
}

function cartesionToBallCentric(v: Vector3, w: Vector3) {
...
export function cueToSpin(offset: Vector3, v: Vector3): Vector3 {
